    ...
    <div class="w-full overflow-x-auto mt-14">
      <table class="w-full table-auto whitespace-no-wrap bg-white border divide-y divide-gray-200 rounded-lg shadow-sm">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="projet in projets" :key="projet.id" class="text-gray-700 hover:bg-gray-100">
            <td class="px-4 py-3 text-sm">{{ projet.id }}</td>
            <td class="px-4 py-3 text-sm">{{ projet.name }}</td>
            <td class="px-4 py-3 text-sm">{{ projet.description }}</td>
            <td class="px-4 py-3 text-sm">{{ projet.date_debut }}</td>
            <td class="px-4 py-3 text-sm">{{ projet.date_fin }}</td>
            <td class="px-4 py-3 text-sm">
              <div class="flex items-center space-x-4">
                <!-- Bouton Éditer -->
                <button @click="editProjet(projet.id)" class="p-2 rounded hover:bg-gray-200">
                  <svg class="w-5 h-5 text-green-600" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                  </svg>
                </button>
                <!-- Bouton Supprimer -->
                <button @click="deleteProjet(projet.id)" class="p-2 rounded hover:bg-gray-200">
...
export default {
  ...
  methods: {
    ...
    async deleteProjet(projetId) {
      try {
        await axios.delete(`http://localhost:8000/api/projets/${projetId}`);
        this.fetchProjets();
      } catch (error) {
